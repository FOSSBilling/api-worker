...
describe("Stats API v1", () => {
  ...
  describe("GET /stats/v1/data", () => {
    it("should return aggregated statistics", async () => {
      ...
      if (data.result.patchesPerRelease.length > 0) {
        ...
        expect(typeof data.result.patchesPerRelease[0].patch_count).toBe("number");
      }
      ...
      if (data.result.releasesPerYear.length > 0) {
        ...
        expect(typeof data.result.releasesPerYear[0].release_count).toBe("number");
      }
    });
  ...
  describe("GET /stats/v1/", () => {
    it("should return HTML page", async () => {
      ...
      expect(html).toContain('src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0');
    });

    it("should include refresh functionality in HTML", async () => {
      const ctx = createExecutionContext();
      const response = await app.fetch(
        new Request("http://localhost/stats/v1"),
        env,
      ...
      expect(html).toContain('onclick="loadStats()"');
      expect(html).toContain('function loadStats()');
    });
  });
});